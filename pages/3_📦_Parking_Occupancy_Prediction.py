...
def preprocess(data, locations, col):
    ...
    st.write(col, 'Space weights for the parking lot:',
             pd.DataFrame(Spatialweight).T)
    ...
    altdata['train_valid'] = ["Train" if x <= len(
        Ytrain) else "Vaild" for x in altdata.index]
    ...
    train_batch_dataset = create_batch_dataset(
        train_dataset, train_labels, batch_size=batch_size)
    test_batch_dataset = create_batch_dataset(
        test_dataset, test_labels, train=False, batch_size=batch_size)
...
def training(col, train_dataset, train_batch_dataset, test_batch_dataset, epochs=30):
    if os.path.exists(os.path.join('./data/output/models/', col)):
        ...
        history = model.fit(train_batch_dataset,
                            ...
                            verbose=0)  # 沉默输出
        model.save(os.path.join('./data/output/models/', col),
                   save_format='h5')
...
def evaluate():
    ...
    if len(ngrok.get_tunnels()) == 0:
        ...
        if sys.platform.startswith('win'):
            # start 开启新进程
            os.system(
                'start tensorboard --logdir ./data/output/logs/fit/ --port 6006')
        elif sys.platform.startswith('linux'):
            # & 开启新进程
            os.system(
                'tensorboard --logdir ./data/output/logs/fit/ --port 6006 &')
    ...
    if len(ngrok.get_tunnels()) == 1:
        if st.button('Reload website'):
            ...
            http_tunnel = ngrok.connect(
                addr='6006', proto='http', bind_tls=True)
        # 若已有网页，则直接获取网页
    ...
    if len(ngrok.get_tunnels()) >= 2:
        # 若网页数量大于2，则清空网页
        st.sidebar.info(
            'TensorBoard failed to start, please refresh the page and try again!')
        for i in ngrok.get_tunnels():
            ngrok.disconnect(i.public_url)
...
def create_batch_dataset(X, y, train=True, buffer_size=100, batch_size=32):
    batch_data = tf.data.Dataset.from_tensor_slices(
        (tf.constant(X), tf.constant(y)))  # 数据封装，tensor类型
    if train:  # 训练集
...
def app():
    ...
    data = pd.read_csv(os.path.join(
        './data/temp/', 'birmingham_time_series.csv'), index_col=0)
    locations = pd.read_csv(os.path.join(
        './data/temp/', 'birmingham_loc_pro.csv'))
    ...
    train_dataset, train_labels, test_dataset, test_labels, train_batch_dataset, test_batch_dataset = preprocess(
        data, locations, col)
    ...
    folium.plugins.HeatMapWithTime(data=labels_list, auto_play=True,
                                   radius=60, display_index=False, name='Raw data').add_to(m.m1)
    folium.plugins.HeatMapWithTime(data=pred_list, auto_play=True,
                                   radius=60, display_index=False, name='Predict data').add_to(m.m2)
